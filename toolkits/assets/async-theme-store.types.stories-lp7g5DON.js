import{r as s,j as e,w as u,s as r,d as f}from"./iframe-BpXj-3b1.js";import{t as y}from"./theme-entry-D4S_RAMB.js";import{d as m}from"./dedent-D4JfOF0A.js";import{T as l}from"./theme-result-card-DQkyr6Gh.js";import"./preload-helper-PPVm8Dsz.js";import"./append-id-Vsg144gU.js";const b=`import type { ThemeEntry } from '../theme-entry.types.ts'
...
`,x={title:"theme/theme-store/AsyncThemeStore",tags:["type","version:1.0"],render:()=>e.jsx(e.Fragment,{})},p={current:"theme-current",grayscale:"theme-grayscale"},d={tags:["props"],decorators:[u({content:e.jsxs(e.Fragment,{children:[e.jsxs("p",{children:["When you specify only the ",e.jsx("code",{children:"read"})," method, the store is read-only."," ",e.jsx("code",{children:"read"})," may return a value or ",e.jsx("code",{children:"Promise"}),"."]}),e.jsxs("p",{children:["When multiple stores have ",e.jsx("code",{children:"read"}),", the first non-empty result is returned."]})]})}),r({source:"read?: () => ThemeEntry | undefined | Promise<ThemeEntry | undefined>"}),r({source:m`
                const store: AsyncThemeStore = {
                    read: async () => themeEntry(themes, 'grayscale')
                }
                await store.read()
            `})],render:()=>{const[t,n]=s.useState(void 0),o={read:async()=>y(p,"grayscale")};return s.useEffect(()=>{o.read?.().then(n)},[]),e.jsx(l,{title:"store.read() (async)","data-testid":"read-only-result",result:t})}},c={tags:["props"],decorators:[u({content:e.jsxs("p",{children:["When you specify only the ",e.jsx("code",{children:"write"})," method, the store is write-only."," ",e.jsx("code",{children:"write"})," may be sync or async (return ",e.jsx("code",{children:"Promise<void>"}),")."]})}),r({source:"write?: (entry: ThemeEntry | undefined) => void | Promise<void>"}),r({source:m`
                const store: AsyncThemeStore = {
                    write: async (entry) => { /* persist */ }
                }
                await store.write?.(themeEntry(themes, 'grayscale'))
            `})],render:()=>{const[t,n]=s.useState(void 0),o={write:async h=>{n(h)}};return s.useEffect(()=>{o.write?.(y(p,"grayscale"))},[]),e.jsx(l,{title:"store.write() receives","data-testid":"write-only-result",result:t})}},a={tags:["props"],decorators:[u({content:e.jsxs(e.Fragment,{children:[e.jsxs("p",{children:["When you specify only the ",e.jsx("code",{children:"subscribe"})," method, the store is subscribe-only.",e.jsx("code",{children:"subscribe"})," is used to observe external changes (e.g. system preference, remote sync)."]}),e.jsxs("p",{children:["The ",e.jsx("code",{children:"handler"})," is called only when the theme changes."]})]})}),r({source:"subscribe?: (handler: (theme: ThemeEntry<Themes> | undefined) => void) => () => void"}),r({source:m`
                const store: AsyncThemeStore = {
                    subscribe: (handler) => {
