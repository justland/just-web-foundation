import{j as e,S as c,w as u,s as m,d as p}from"./iframe-BpXj-3b1.js";import{a as t}from"./append-id-Vsg144gU.js";import{d as f}from"./dedent-D4JfOF0A.js";import"./preload-helper-PPVm8Dsz.js";const{expect:a}=__STORYBOOK_MODULE_TEST__,I={title:"utils/appendId",tags:["func","version:1.0"],parameters:p({description:{component:"Appends a suffix to an ID if the ID is defined. Useful for propagating ids (e.g. data-testid, key, or aria attributes) from parent to sub-components so selectors and keys stay consistent and predictable."}}),render:()=>e.jsx(e.Fragment,{})},r={tags:["use-case"],parameters:p({description:{story:"Append a suffix to a defined ID. Returns the combined id with a hyphen."}}),decorators:[u(),m({source:f`
                appendId('card', 'title')   // 'card-title'
                appendId('form', 'submit')  // 'form-submit'
            `})],render(){const d=[{id:"card",suffix:"title"},{id:"form",suffix:"submit"},{id:"list",suffix:"item-0"}];return e.jsx(c,{appearance:"output",children:e.jsx("pre",{className:"text-sm",children:d.map(({id:i,suffix:n})=>`appendId('${i}', '${n}') → ${JSON.stringify(t(i,n))}`).join(`
`)})})},play:async()=>{await a(t("card","title")).toBe("card-title"),await a(t("form","submit")).toBe("form-submit")}},s={name:"data-testid and key propagation",tags:["use-case"],parameters:p({description:{story:"Pass a root id (e.g. from props) down and use appendId for data-testid, key, or other ids so sub-components get stable, predictable identifiers without manual concatenation."}}),decorators:[u(),m({source:f`
            ...
            `})],render(){const d="product-card",i=["title","item-0","item-1","item-2"];return e.jsxs(c,{appearance:"output",children:[e.jsxs("p",{className:"mb-2 text-sm text-gray-600 dark:text-gray-400",children:["With ",e.jsxs("code",{children:['id="',d,'"']}),", sub-ids become:"]}),e.jsx("pre",{className:"text-sm",children:i.map(n=>`appendId('${d}', '${n}') → ${t(d,n)}`).join(`
`)})]})},play:async()=>{const d="product-card";await a(t(d,"title")).toBe("product-card-title"),await a(t(d,"item-0")).toBe("product-card-item-0")}},o={name:"when id is undefined",tags:["unit"],parameters:p({description:{story:"When the id is undefined, appendId returns undefined. Use this when the parent does not set an id (e.g. optional data-testid) so sub-components do not render stray ids."}}),decorators:[u(),m({source:f`
                appendId(undefined, 'title')  // undefined
                appendId(undefined, 'item')   // undefined
            `})],render(){return e.jsx(c,{appearance:"output",children:e.jsxs("pre",{className:"text-sm",children:[`appendId(undefined, 'title') → ${JSON.stringify(t(void 0,"title"))}`,`
