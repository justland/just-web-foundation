...
`,{expect:c}=__STORYBOOK_MODULE_TEST__,me={title:"theme/theme-store/cookieThemeStore",tags:["func","version:1.0"],parameters:h({description:{component:"Theme store backed by cookies. Persists across sessions; SSR-readable. Cross-tab sync is not supported."}}),render:()=>s.jsx(s.Fragment,{})},n={current:"theme-current",next:"theme-next",grayscale:"theme-grayscale","high-contrast":"theme-high-contrast"},a="theme-cookie-demo";function _(e){document.cookie=`${e}=; path=/; max-age=0`}const T={tags:["playground"],parameters:h({description:{story:"Interactive demo: Read theme, write themes via buttons, and observe subscribe updates."}}),decorators:[l(),p({source:g`
                const store = cookieThemeStore(themes, { cookieName: 'app-theme' })
                <ThemeStoreDemo2 store={store} themes={themes} />
            `})],loaders:[()=>(_(a),{})],render:()=>{const e=H.useMemo(()=>u(n,{cookieName:a}),[]);return s.jsx($,{store:e,themes:n})}},R="theme-cookie-parse";function G(){document.cookie=`${R}=; path=/; max-age=0`}function V(e,t){let o;try{o=t?JSON.parse(t):void 0}catch{return}if(!o?.theme||typeof o.theme!="string"||!(o.theme in e))return;const r=o.theme;return{theme:r,value:e[r]}}const v={name:"options.parse",tags:["props","use-case"],parameters:h({description:{story:"The options.parse allows you to provide a custom parse function to parse the stored value into a structure you need. Use it when migrating from legacy formats, supporting custom serialization, or relaxing validation. This example accepts legacy format { theme } (no value field) and coerces to ThemeEntry; the default parseStoredTheme would return undefined."}}),decorators:[l({content:s.jsxs(s.Fragment,{children:[s.jsxs("p",{children:["The ",s.jsx("code",{children:"options.parse"})," allows you to provide a custom parse function to parse the stored value to a specific structure you wanted. Use it when migrating from legacy formats, supporting custom serialization, or relaxing validation."]}),s.jsxs("p",{children:["The example below pre-seeds the cookie with legacy format"," ",s.jsx("code",{children:'{ theme: "grayscale" }'})," (no value)."]})]})}),p({source:g`
...
}`,...w.parameters?.docs?.source}}};const ce=["Playground","ParseOption","CookieName","Read","ReadWhenEmpty","GetThemeFromCookie","Source"];export{S as CookieName,N as GetThemeFromCookie,v as ParseOption,T as Playground,C as Read,x as ReadWhenEmpty,w as Source,ce as __namedExportsOrder,me as default};
