import{j as r,d as a,w as o,s as c,S as u}from"./iframe-CuGcEWf0.js";import{d}from"./dedent-BuYMbVyj.js";import{p}from"./px-2-rem-CPoMxM9B.js";import"./preload-helper-PPVm8Dsz.js";const S=`/**
...
`,{expect:i}=__STORYBOOK_MODULE_TEST__,B={title:"units/px2rem",tags:["func","version:3.1"],parameters:a({description:{component:'Converts pixel values to rem units. Accepts a number or string (e.g. "16px" or "16") and optional base (default 16) and precision (default 4).'}}),render:()=>r.jsx(r.Fragment,{})},x={tags:["use-case"],parameters:a({description:{story:"Convert pixel numbers to rem using the default base (16px)."}}),decorators:[o({content:r.jsx(r.Fragment,{children:r.jsxs("p",{children:[r.jsx("code",{children:"px2rem(px)"})," returns the rem value. Default base is 16."]})})}),c({source:d`
                px2rem(16)  // 1
                px2rem(32)  // 2
                px2rem(8)   // 0.5
                px2rem(24)  // 1.5
...
`)})})},play:async()=>{const t=[{input:16,expected:1},{input:32,expected:2},{input:8,expected:.5},{input:24,expected:1.5}];for(const{input:e,expected:n}of t)await i(p(e)).toBe(n)}},m={tags:["use-case"],parameters:a({description:{story:'String inputs like "16px" or "16" are parsed and converted.'}}),decorators:[o(),c({source:d`
                px2rem('16px')  // 1
                px2rem('32px')  // 2
                px2rem('24')    // 1.5
                px2rem('8.5px') // 0.5313
...
`)})})},play:async()=>{const t=[{input:"16px",expected:1},{input:"32px",expected:2},{input:"24",expected:1.5},{input:"8.5px",expected:.5313}];for(const{input:e,expected:n}of t)await i(p(e)).toBe(n)}},l={tags:["use-case"],parameters:a({description:{story:"Pass a custom base (pixels per 1rem) via options."}}),decorators:[o(),c({source:d`
                px2rem(20, { base: 20 })  // 1
                px2rem(40, { base: 20 })  // 2
...
`)})})},play:async()=>{const t=[{input:20,base:20,expected:1},{input:40,base:20,expected:2},{input:10,base:20,expected:.5},{input:30,base:20,expected:1.5}];for(const{input:e,base:n,expected:s}of t)await i(p(e,{base:n})).toBe(s)}},b={tags:["use-case"],parameters:a({description:{story:"Control decimal places with the precision option."}}),decorators:[o(),c({source:d`
...
`)})})},play:async()=>{const e=[1,.8,.81,.813,.8125,.8125],n=[0,1,2,3,4,6];for(let s=0;s<n.length;s++)await i(p(13,{precision:n[s]})).toBe(e[s])}},f={tags:["use-case"],parameters:a({description:{story:"Combine custom base and precision."}}),decorators:[o(),c({source:d`
...
px2rem(undefined) → ${p(void 0)}`})})},play:async()=>{await i(p(null)).toBe(null),await i(p(void 0)).toBe(void 0)}},g={tags:["source"],parameters:a({source:{code:S}}),decorators:[c()]};x.parameters={...x.parameters,docs:{...x.parameters?.docs,source:{originalSource:`{
  tags: ['use-case'],
