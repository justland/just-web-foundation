...
import { sessionStorageThemeStore } from '#just-web/toolkits'
import { ThemeResultCard } from '../testing/theme-result-card.tsx'
import source from './session-storage-theme-store.ts?raw'

const meta = {
	title: 'theme/sessionStorageThemeStore',
	tags: ['func', 'version:next'],
	parameters: defineDocsParam({
		description: {
			component:
				'Theme store backed by sessionStorage for a fixed storage key. The store provides get, set, and subscribe that use the given key. Callers pass themes and optional default theme when calling get/set/subscribe.'
		}
	}),
	render: () => <></>
} satisfies Meta
...
function StoreGetDemo({
	...
	const store = sessionStorageThemeStore<typeof themes>(storageKey)
