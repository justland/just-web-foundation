...
export const Playground: Story = {
	...
	render: () => {
		const store = sessionStorageThemeStore(themes, { storageKey: STORAGE_KEY })
...
export const ThemeMapStringValue: Story = {
	...
	loaders: [
		() => {
			...
			const store = sessionStorageThemeStore(themes, { storageKey: THEMEMAP_STORAGE_KEY })
			store.write(themeEntry(themes, 'current'))
			return { store }
		}
	],
...
export const ThemeMapArrayValues: Story = {
	...
	loaders: [
		() => {
			window.sessionStorage.removeItem(THEMEMAP_STORAGE_KEY)
			const store = sessionStorageThemeStore(themesArray, { storageKey: THEMEMAP_STORAGE_KEY })
...
const CUSTOM_PARSE_STORAGE_KEY_SS = 'theme-ss-custom-parse'
...
export const ParseOption: Story = {
	...
	loaders: [
		() => {
			window.sessionStorage.removeItem(CUSTOM_PARSE_STORAGE_KEY_SS)
			const store = sessionStorageThemeStore(themesLightDark, {
				storageKey: CUSTOM_PARSE_STORAGE_KEY_SS,
				parse: parseThemeEntryWithTimestamp
			})
			store.write({
				theme: 'dark',
...
export const Subscribe: Story = {
	...
	render: () => {
		const store = useMemo(() => sessionStorageThemeStore(themes, { storageKey: STORAGE_KEY }), [])
