...
const CUSTOM_PARSE_STORAGE_KEY = 'theme-ls-custom-parse'
...
export const ParseOption: Story = {
	...
	decorators: [
		...
		showSource({
			source: dedent`
				...
				// Write with extra data
				store.write({
					theme: 'dark',
					value: { themeValue: 'theme-dark', timestamp: Date.now() }
				})

				// Read back - value includes timestamp
	...
	loaders: [
		() => {
			window.localStorage.removeItem(CUSTOM_PARSE_STORAGE_KEY)
			const store = localStorageThemeStore(themesLightDark, {
				storageKey: CUSTOM_PARSE_STORAGE_KEY,
				parse: parseThemeEntryWithTimestamp
			})
			store.write({
				theme: 'dark',
