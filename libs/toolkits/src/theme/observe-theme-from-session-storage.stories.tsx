...
import { observeThemeFromSessionStorage, setThemeToSessionStorage } from '#just-web/toolkits'
import { defineThemeStorageOptions } from './define-theme-storage-options.ts'
import source from './observe-theme-from-session-storage.ts?raw'

const meta = {
	title: 'theme/observeThemeFromSessionStorage',
	tags: ['func', 'version:next'],
	parameters: defineDocsParam({
		description: {
			component:
				'Observes the theme stored in sessionStorage and invokes a handler with the current theme. The handler is called once on start and when another tab/window changes the same storage key.',
		},
	}),
	render: () => <></>,
} satisfies Meta
...
const STORAGE_KEY = 'theme-observe-session'
...
function ObserveThemeDemo({
	...
	return (
		<StoryCard title="Observed theme from sessionStorage" appearance="output">
