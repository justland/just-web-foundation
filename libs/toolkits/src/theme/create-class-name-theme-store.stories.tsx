...
import { createClassNameThemeStore } from '#just-web/toolkits'
import { ThemeResultCard } from '../testing/theme-result-card.tsx'
import source from './create-class-name-theme-store.ts?raw'

const meta = {
	title: 'theme/createClassNameThemeStore',
	tags: ['func', 'version:next'],
	parameters: defineDocsParam({
		description: {
			component:
				'Creates a theme store that reads and writes theme via element class names. The store provides get, set, and subscribe for the given element (or document.documentElement when omitted).',
		},
	}),
	render: () => <></>,
} satisfies Meta
...
function StoreGetDemo({
	...
	const store = createClassNameThemeStore<typeof themes>()
...
export const SameElementReturnsCachedStore: Story = {
	...
	parameters: defineDocsParam({
		description: {
			story:
				'Calling createClassNameThemeStore with the same element (or both undefined for documentElement) returns the same store instance.',
		},
	}),
	loaders: [
		() => {
			const store1 = createClassNameThemeStore<typeof themes>()
			const store2 = createClassNameThemeStore<typeof themes>()
	...
	decorators: [
		...
		showSource({
			source: dedent`
				const store1 = createClassNameThemeStore()
				const store2 = createClassNameThemeStore()
				store1 === store2
			`,
		}),
	],
