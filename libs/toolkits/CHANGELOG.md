...
- 5e6a487: Pin `type-plus` to the exact version `8.0.0-beta.10` instead of a caret range.
...
- c7745d5: Depend on `type-plus@^8.0.0-beta.10` — a range on the 8.x line, not an exact pin.
...
- 51a0c8d: Depend on `type-plus@^7.6.2` instead of an `8.0.0-beta` prerelease.
...
- d989aaa: Add the remaining CSS user-preference media features alongside `prefers-color-scheme`
...
- 9022a34: Add function overloads for null/undefined inputs in CSS unit utilities to preserve literal return types. `convertCssUnit`, `createCssUnitConverter`, `getCssUnit`, `isEffectivelyZero`, `parseCssNumber`, `parseCssValue`, `px2rem`, and `rem2px` now narrow correctly when given `null` or `undefined`.
